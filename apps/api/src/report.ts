...
/** Human label for the configured retention window (env SCAN_RETENTION_MINUTES). */
function retentionLabel(): string {
  const m = Number(process.env.SCAN_RETENTION_MINUTES) || 60;
  if (m % 60 === 0) {
    const h = m / 60;
    return `${h} hour${h === 1 ? '' : 's'}`;
  }
...
<body><div class="wrap">
  ...
  <div class="foot">Generated by QA Prism on ${fmt(new Date())} · reports are retained for ${esc(retentionLabel())}</div>
</div></body></html>`;
}
